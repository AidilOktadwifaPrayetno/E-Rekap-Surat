...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if nama_lengkap and jabatan_id combination already exists
    $check_nama_jabatan = mysqli_query($conn, "SELECT * FROM karyawan WHERE nama_lengkap = '$nama_lengkap' AND jabatan_id = $jabatan_id");

    if(mysqli_num_rows($check_nama_jabatan) > 0) {
        echo "<script>
                document.addEventListener('DOMContentLoaded', function() {
                    Swal.fire({
                        title: 'Gagal!',
                        text: 'Nama lengkap dengan jabatan tersebut sudah terdaftar!',
                        icon: 'error',
